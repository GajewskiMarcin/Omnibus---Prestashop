...
class MgOmnibus extends Module
{
    ...
    private function setDefaultConfiguration()
    {
        ...
        Configuration::updateValue('MGOMNIBUS_CRON_TOKEN', Tools::passwdGen(16));
        $default_css = ".mgomnibus {\n    font-size: 0.9em;\n    color: #7a7a7a;\n    clear: both;\n    margin-top: 5px;\n}\n.mgomnibus--listing {\n    font-size: 0.8em;\n}\n.mgomnibus__percent {\n    display: inline-block;\n    background-color: #f13340;\n    color: #fff;\n    padding: 2px 5px;\n    border-radius: 3px;\n    font-weight: bold;\n    margin-left: 5px;\n    font-size: 0.9em;\n}";
        Configuration::updateValue('MGOMNIBUS_CUSTOM_CSS', $default_css);
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit' . $this->name)) {
            if ($this->postProcess()) {
                ...
                $output .= $this->displayError($this->l('An error occurred while saving.'));
            }
        }
    ...
    protected function getGeneralConfigForm()
    {
        return [
            'form' => [
                ...
                'input' => [
                    ['type' => 'text', 'label' => $this->l('Period in days'), 'name' => 'MGOMNIBUS_DAYS', 'desc' => $this->l('Number of days to check for the lowest price (default 30).'), 'class' => 'fixed-width-xs'],
                    ['type' => 'select', 'label' => $this->l('Display mode'), 'name' => 'MGOMNIBUS_MODE', 'options' => ['query' => [['id' => 'always', 'name' => $this->l('Always')], ['id' => 'only_discount', 'name' => $this->l('Only on discount')]], 'id' => 'id', 'name' => 'name']],
                    ['type' => 'select', 'label' => $this->l('Price type'), 'name' => 'MGOMNIBUS_PRICE_KIND', 'options' => ['query' => [['id' => 'gross', 'name' => $this->l('Gross (tax incl.)')], ['id' => 'net', 'name' => $this->l('Net (tax excl.)')]], 'id' => 'id', 'name' => 'name']],
                    ...
                    ['type' => 'text', 'label' => $this->l('Precision (decimal places)'), 'name' => 'MGOMNIBUS_PRECISION', 'class' => 'fixed-width-xs', 'desc' => $this->l('Used for price comparison and discount percentage display.')],
                    ['type' => 'text', 'label' => $this->l('Label on product page'), 'name' => 'MGOMNIBUS_PRODUCT_LABEL', 'lang' => true],
                    ['type' => 'text', 'label' => $this->l('Label on product listing'), 'name' => 'MGOMNIBUS_LISTING_LABEL', 'lang' => true],
            ...
            'form' => [
                ...
                'input' => [
                    ...
                    [
                        ...
                        'values' => [
                            'query' => [
                                ['id' => 'ALL_COUNTRIES', 'name' => $this->l('Log for all countries'), 'val' => 1],
                                ['id' => 'ALL_GROUPS', 'name' => $this->l('Log for all customer groups'), 'val' => 1],
                            ],
                            'id' => 'id',
                            'name' => 'name',
                        ],
                        'desc' => $this->l('Uncheck to log prices only for the default country/group, which will reduce the database size.'),
                    ],
                    ['type' => 'text', 'label' => $this->l('Log retention (days)'), 'name' => 'MGOMNIBUS_RETENTION', 'desc' => $this->l('After how many days old price logs should be deleted by CRON.'), 'class' => 'fixed-width-sm'],
                    ['type' => 'text', 'label' => $this->l('CRON Token'), 'name' => 'MGOMNIBUS_CRON_TOKEN', 'desc' => $this->l('Your CRON URL: ') . '<br><code>' . $cron_url . '</code><br>' . $this->l('Add &limit=all to process all products at once.'), 'class' => 'fixed-width-xxl']
                ],
                'submit' => ['title' => $this->l('Save')],
            ],
        ];
    ...
    protected function postProcess()
    {
        ...
        foreach ($form_inputs as $input) {
             if ($input['type'] === 'checkbox') {
                 foreach ($input['values']['query'] as $value) {
                     Configuration::updateValue('MGOMNIBUS_LOG_' . $value['id'], (bool)Tools::getValue('MGOMNIBUS_LOG_' . $value['id']));
                 }
             } elseif (!empty($input['name'])) {
                $isHtml = (isset($input['type']) && $input['type'] == 'textarea');
                if (isset($input['lang']) && $input['lang']) {
                    $lang_values = [];
                    foreach ($languages as $lang) {
                        $lang_values[$lang['id_lang']] = Tools::getValue($input['name'] . '_' . $lang['id_lang']);
                    }
                    Configuration::updateValue($input['name'], $lang_values, $isHtml);
                } else {
                    Configuration::updateValue($input['name'], Tools::getValue($input['name']), $isHtml);
                }
             }
        }
        return true;
    ...
    private function renderOmnibusInfo($productData, $viewType)
    {
        ...
        $price_kind_conf = Configuration::get('MGOMNIBUS_PRICE_KIND');
        $useTax = ($price_kind_conf == 'gross');
        $precision = (int)Configuration::get('MGOMNIBUS_PRECISION');
        ...
        $days = (int)Configuration::get('MGOMNIBUS_DAYS', null, null, $id_shop, 30);
        ...
        if (count($distinctPrices) === 1) {
            ...
            if (Tools::ps_round($currentPrice, 6) < Tools::ps_round($lowestPrice, 6)) {
                ...
                $referencePrice = $secondLowestPrice;
            } else {
                $absoluteMinPrice = $lowestPrice;
                $referencePrice = $lowestPrice;
            }
        ...
        $displayMode = Configuration::get('MGOMNIBUS_MODE', null, null, $id_shop, 'only_discount');
        $shouldDisplay = false;
        
        $percentChange = 0;
        if ($referencePrice > 0 && Tools::ps_round($currentPrice, $precision) < Tools::ps_round($referencePrice, $precision)) {
             $percentChange = round((1 - ($currentPrice / $referencePrice)) * 100, $precision);
        }
        ...
        if ($priceToDisplay > 0) {
            if ($displayMode === 'always') {
                $shouldDisplay = true;
            } elseif ($displayMode === 'only_discount') {
                if ($percentChange > 0) {
                    $shouldDisplay = true;
                }
            }
        ...
        $this->smarty->assign('mgomnibus', [
            'text' => Configuration::get('MGOMNIBUS_' . strtoupper($viewType) . '_LABEL', $this->context->language->id),
            'min_price' => $priceToDisplay,
            'min_price_formatted' => Tools::displayPrice($priceToDisplay, $this->context->currency),
            'days' => $days,
            'kind' => $price_kind_conf,
            'has_percent' => $percentChange > 0 && Configuration::get('MGOMNIBUS_SHOW_PERCENT'),
